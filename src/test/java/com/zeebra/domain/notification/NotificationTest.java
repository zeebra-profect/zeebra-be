...
public class NotificationTest {
    ...
    @Test
    @DisplayName("TC-IT-NOTI-001-[정상] 유효한 memberId와 Type으로 알림 생성 시 DB에 저장됨")
    public void createNotification_validInput_success() {
        // given
        Member member1 = createTestMember("user1", "user1@abc.a");
        NotificationRequest request = new NotificationRequest(member1.getId(), NotificationType.TEST, null);
    ...
    @Test
    @DisplayName("TC-IT-NOTI-002-[예외] 잘못된 memberId 입력 시 알림 생성 실패")
    ...
    @Test
    @DisplayName("TC-IT-NOTI-003-[예외] null Type 입력 시 알림 생성 실패")
    public void createNotification_invalidType_fail() {
        // given
        Member member1 = createTestMember("user1", "user1@abc.a");

        NotificationRequest request = new NotificationRequest(member1.getId(), null, null);
    ...
    @Test
    @DisplayName("TC-IT-NOTI-004-[정상] object, url 포함 알림 생성")
    public void createNotification_validObjectAndUrl_success() {
        // given
        Member member1 = createTestMember("user1", "user1@abc.a");

        NotificationRequest request = new NotificationRequest(member1.getId(), NotificationType.TEST_OBJECT, new Object());
        ...
        assertThat(savedNotification.get().getNotificationType()).isEqualTo(request.getNotificationType());
        assertThat(savedNotification.get().getMemberId()).isEqualTo(request.getMemberId());
        assertThat(savedNotification.get().getUrl()).isEqualTo("/dummy/testId");
    }

    @Test
    @DisplayName("TC-IT-NOTI-005-[정상] 여러 사용자에게 알림 생성")
    public void createNotification_multipleNotificationsForDifferentMembers_success() {
        ...
        // when
        notificationService.createNotification(new NotificationRequest(member1.getId(), NotificationType.TEST, null));
        notificationService.createNotification(new NotificationRequest(member2.getId(), NotificationType.TEST, null));
        notificationService.createNotification(new NotificationRequest(member3.getId(), NotificationType.TEST, null));

        // then
        assertThat(notificationRepository.findByNotificationTypeAndMemberId(NotificationType.TEST, member1.getId())).isNotNull();
        assertThat(notificationRepository.findByNotificationTypeAndMemberId(NotificationType.TEST, member2.getId())).isNotNull();
        assertThat(notificationRepository.findByNotificationTypeAndMemberId(NotificationType.TEST, member3.getId())).isNotNull();
    }
    ...
    public void getNotifications_validMemberId_success() {
        ...
        // when
        NotificationsResponse response = notificationService.getNotifications(member1.getId());
    ...
    @Test
    @DisplayName("TC-IT-NOTI-010-[정상] 특정 알림 단건 조회 성공")
    public void getNotificationById_validNotificationId_success() {
        // given
        Member member = createTestMember("user1", "user1@abc.a");
        ...
        assertThat(foundNotification.memberId()).isEqualTo(member.getId());
